 ...
 * @version 1.0
 */
namespace Whalin.Caching.Memcached
{
    using System;
    ...
    /// <summary>
    /// This class is a connection sockIOPool for maintaning a sockIOPool of persistent connections
    /// to memcached servers.
    /// 
    /// The sockIOPool must be initialized prior to use. This should typically be early on
    ...
    /// boolean nagleAlg        = false;			// turn off Nagle's algorithm on all sockets in sockIOPool	
    /// 
    /// SockIOPool sockIOPool = SockIOPool.getInstance();
    /// sockIOPool.Servers = serverlist;
    /// sockIOPool.Weights = weights;	
    /// sockIOPool.InitConnections = initialConnections;
    /// sockIOPool.MinConnections = minSpareConnections;
    /// sockIOPool.MaxConnections = maxSpareConnections;
    /// sockIOPool.MaxIdle = maxIdleTime;
    /// sockIOPool.MaxBusy = maxBusyTime;
    /// sockIOPool.MaintenanceSleep = maintThreadSleep;
    /// sockIOPool.SocketTimeout = socketTimeOut;
    /// sockIOPool.SocketConnectTimeOut = socketConnectTO;	
    /// sockIOPool.Nagle = nagleAlg;	
    /// sockIOPool.HashingAlg = SockIOPool.HashingAlgorithms.NEW_COMPAT_HASH;	
    /// sockIOPool.Initialize();	
    /// 
    /// 
    /// //The easiest manner in which to initialize the sockIOPool is to set the servers and rely on defaults as in the first example.
    /// //After sockIOPool is initialized, a memcachedClient will request a SockIO object by calling getSock with the cache key
    /// //The memcachedClient must always close the SockIO object when finished, which will return the connection back to the sockIOPool.
    ...
    public class SockIOPool
    {
        ...
        private int _maxCreate = 1;	// this will be initialized by sockIOPool when the sockIOPool is initialized
        private Hashtable _createShift;

        // initial, min and max sockIOPool sizes
        ...
        // sockIOPool name
        private string _poolName = "memcached";

        /// <summary>
        /// Factory to create/retrieve new pools given a unique poolName.
        /// </summary>
        /// <param name="poolName">unique name of the sockIOPool</param>
        ...
        /// Simply creates a sockIOPool named "default". 
        /// </summary>
        /// <returns>instance of SockIOPool</returns>
        [MethodImpl(MethodImplOptions.Synchronized)]
        ...
        /// <summary>
        /// Gets or sets the initial number of connections per server setting in the available sockIOPool.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the minimum number of spare connections to maintain in our available sockIOPool
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the maximum number of spare connections allowed in our available sockIOPool.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the maximum idle time for threads in the avaiable sockIOPool.
        /// </summary>
        public long MaxIdle
        {
            get { return _maxIdle; }
        ...
        /// <summary>
        /// Gets or sets the maximum busy time for threads in the busy sockIOPool
        /// </summary>
        ...
        public void Initialize() 
		{
				...
				&& _busyPool != null) 
			{
                ...
                //{
                //    Log.Error(GetLocalizedString("initializing initialized sockIOPool"));
                //}
				return;
			}
            ...
            //{
            //    Log.Debug(GetLocalizedString("initializing sockIOPool")
			...
			// mark sockIOPool as initialized
			_initialized = true;
        ...
        /// <summary>
        /// Returns the state of the sockIOPool
        /// </summary>
        ...
        /// 
        /// Trys to get SockIO from sockIOPool.  Fails over
        ...
        public SockIO GetSock(string key, object hashCode)
        {
            ...
            if(!_initialized)
            {
                //if(Log.IsErrorEnabled)
                //{
                //    Log.Error(GetLocalizedString("get socket from uninitialized sockIOPool"));
                //}
                return null;
            }
        ...
        /// <summary>
        /// Returns a SockIO object from the sockIOPool for the passed in host.
        /// 
        ...
        public SockIO GetConnection(string host)
        {
            ...
            // if we have items in the sockIOPool
                ...
                if(aSockets != null && !(aSockets.Count == 0))
                {
                    foreach(SockIO socket in new IteratorIsolateCollection(aSockets.Keys))
                    {
                        if(socket.IsConnected)
                        {
                            ...
                            // remove from avail sockIOPool
                            aSockets.Remove(socket);

                            // add to busy sockIOPool
            ...
            // if here, then we found no sockets in the sockIOPool
            ...
            for(int i = create; i > 0; i--)
            {
                ...
                if(i == 1)
                {
                    // last iteration, add to busy sockIOPool and return sockio
                    AddSocketToPool(_busyPool, host, socket);
                    return socket;
                }
                else
        ...
        /// <summary>
        /// Removes a socket from specified sockIOPool for host.
        /// 
        /// Internal utility method. 
        /// </summary>
        /// <param name="sockIOPool">sockIOPool to remove from</param>
        /// <param name="host">host sockIOPool</param>
        ...
        /// <summary>
        /// Closes and removes all sockets from specified sockIOPool for host. 
        /// 
        /// Internal utility method. 
        /// </summary>
        /// <param name="sockIOPool">sockIOPool to clear</param>
        ...
        /// <summary>
        /// Checks a SockIO object in with the sockIOPool.
        /// 
        /// This will remove SocketIO from busy sockIOPool, and optionally
        /// add to avail sockIOPool.
        /// </summary>
        /// <param name="socket">socket to return</param>
        /// <param name="addToAvail">add to avail sockIOPool if true</param>
        [MethodImpl(MethodImplOptions.Synchronized)]
        ...
        /// <summary>
        /// Returns a socket to the avail sockIOPool.
        /// 
        ...
        /// <summary>
        /// Closes all sockets in the passed in sockIOPool.
        /// 
        /// Internal utility method. 
        /// </summary>
        /// <param name="sockIOPool">sockIOPool to close</param>
        ...
        public void Shutdown()
        {
            //if(Log.IsDebugEnabled)
            //{
            //    Log.Debug(GetLocalizedString("start socket sockIOPool shutdown"));
            //}
            ...
            //if(Log.IsDebugEnabled)
            //{
            //    Log.Debug(GetLocalizedString("end socket sockIOPool shutdown"));
            //}
        }
        ...
        /// 
        /// This thread will manage the size of the active sockIOPool
        /// as well as move any closed, but not checked in sockets
        /// back to the available sockIOPool.
        /// </summary>
        [MethodImpl(MethodImplOptions.Synchronized)]
        ...
        /// 
        /// This is typically called by the maintenance thread to manage sockIOPool size. 
        /// </summary>
        [MethodImpl(MethodImplOptions.Synchronized)]
        protected void SelfMaintain()
        {
            ...
            // as needed to maintain sockIOPool settings
            foreach(string host in new IteratorIsolateCollection(_availPool.Keys))
            {
                Hashtable sockets = (Hashtable)_availPool[host];
                //if(Log.IsDebugEnabled)
                //{
                //    Log.Debug(GetLocalizedString("size of available sockIOPool").Replace("$$Host$$", host).Replace("$$Sockets$$", sockets.Count.ToString(new NumberFormatInfo())));
                //}

                // if sockIOPool is too small (n < minSpare)
            ...
            foreach(string host in _busyPool.Keys)
            {
                ...
                //if(Log.IsDebugEnabled)
                //{
                //    Log.Debug(GetLocalizedString("size of busy sockIOPool").Replace("$$Host$$", host).Replace("$$Sockets$$", sockets.Count.ToString(new NumberFormatInfo())));
                //}
            ...
            private void Maintain()
            {
                while(!_stopThread)
                {
                    try
                    {
                        ...
                        // if sockIOPool is initialized, then
        ...
        private static ResourceManager _resourceManager = new ResourceManager("LightFramework.Caching.Memcached.Whalin.Caching.Memcached.StringMessages", typeof(SockIOPool).Assembly);
