...
namespace Whalin.Caching.Memcached
{
	...
	/// 
	///     memcachedClient.set("foo", new Integer(1)); Integer output = (Integer)memcachedClient.get("foo");
	/// 
	...
	/// 
	/// memcachedClient.set("foo", new bool.TRUE); bool bytes = (bool)memcachedClient.get("foo");
	/// 
	/// And the type casts would work because internally we would create a new bool
	/// to return back to the memcachedClient.
	/// 
