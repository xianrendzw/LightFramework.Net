...
namespace Whalin.Caching.Memcached
{
	...
	/// <summary>
	/// This is a C# memcachedClient for the memcached server available from
	...
	///
	/// Now pulls SockIO objects from SockIOPool, which is a connection sockIOPool.  The server failover
	/// has also been moved into the SockIOPool class.
	/// This sockIOPool needs to be initialized prior to the memcachedClient working.  See javadocs from SockIOPool.
	/// (This will have to be fixed for our C# version.  Most of this code is straight ported over from Java.)
	/// </summary>
	/// <example>
	/// //***To create cache memcachedClient object and set params:***
	...
	///	memcachedClient.setCompressThreshold(4096);
	///
	...
	///	object value = SomeClass.getObject();	
	///	int hash = 45;
	///	memcachedClient.set(key, value, hash);
	/// 
	/// 
	...
	/// string key   = "cacheKey1";	
	/// int hash = 45;
	/// memcachedClient.delete(key, hashCode);
	/// 
	/// 
	/// //***To store a counter and then increment or decrement that counter:***
	/// MemcachedClient memcachedClient = new MemcachedClient();
	/// string key   = "counterKey";	
	/// memcachedClient.storeCounter(key, 100);
	/// Console.WriteLine("counter after adding      1: " memcachedClient.incr(key));	
	/// Console.WriteLine("counter after adding      5: " memcachedClient.incr(key, 5));	
	/// Console.WriteLine("counter after subtracting 4: " memcachedClient.decr(key, 4));	
	/// Console.WriteLine("counter after subtracting 1: " memcachedClient.decr(key));	
	/// 
	/// 
	...
	/// string key   = "counterKey";	
	/// int hash = 45;	
	/// memcachedClient.storeCounter(key, 100, hash);
	/// Console.WriteLine("counter after adding      1: " memcachedClient.incr(key, 1, hash));	
	/// Console.WriteLine("counter after adding      5: " memcachedClient.incr(key, 5, hash));	
	/// Console.WriteLine("counter after subtracting 4: " memcachedClient.decr(key, 4, hash));	
	/// Console.WriteLine("counter after subtracting 1: " memcachedClient.decr(key, 1, hash));	
	/// 
	/// 
	...
	/// string key   = "key";	
	/// int hash = 45;	
	/// object value = memcachedClient.get(key, hash);
	/// 
	/// 
	...
	/// string[] keys    = { "key", "key1", "key2" };
	/// int[] hashes = { 45, 32, 44 };
	/// object value = memcachedClient.getMulti(keys, hashes);
	/// 
	///
	...
	/// MemcachedClient memcachedClient = new MemcachedClient();
	/// Hashtable stats = memcachedClient.stats();
	/// </example>
	public class MemcachedClient 
	{
        ...
        private const string ERROR = "ERROR"; // invalid command name from memcachedClient
        private const string CLIENT_ERROR = "CLIENT_ERROR"; // memcachedClient error in input line - invalid protocol
		...
		/// <summary>
		/// Initializes memcachedClient object to defaults.
		/// 
		...
		public bool Delete(string key, object hashCode, DateTime expiry) 
		{
			...
			try 
			{
				...
				if(DELETED == line) 
				{
					...
					// return sock to sockIOPool and bail here
					sock.Close();
					sock = null;
					return true;
				}
			...
			else 
			{
                if(obj != null)
                {
                    ...
                    catch//(IOException e)
                    {
                        ...
                        // return socket to sockIOPool and bail
                        sock.Close();
                        sock = null;
                        return false;
                    }
		...
		private long IncrementOrDecrement(string cmdname, string key, long inc, object hashCode) 
		{
			...
			try 
			{
				...
				if(new Regex("\\d+").Match(line).Success) 
				{
					...
					// return sock to sockIOPool and return result
					sock.Close();
		...
		public bool FlushAll(ArrayList servers) 
		{
			...
			if(pool == null) 
			{
                //if(log.IsErrorEnabled)
                //{
                //    log.Error(GetLocalizedString("unable to get socket sockIOPool"));
                //}
				return false;
			}
        ...
        private static ResourceManager _resourceManager = new ResourceManager("LightFramework.Caching.Memcached.Whalin.Caching.Memcached.StringMessages", typeof(MemcachedClient).Assembly);
