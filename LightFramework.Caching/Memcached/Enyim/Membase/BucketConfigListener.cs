...
namespace Enyim.Membase
{
	internal class BucketConfigListener
	{
		...
		/// <summary>
		/// Connection timeout in milliseconds for connecting the sockIOPool.
		/// </summary>
		...
		/// <summary>
		/// Time to wait in milliseconds to reconnect to the sockIOPool when all nodes are down.
		/// </summary>
		public int DeadTimeout { get; set; }

		/// <summary>
		/// Raised when the sockIOPool's configuration changes.
		/// </summary>
		...
		private void RaiseConfigChanged(ClusterConfig config)
		{
			...
			// we got a new config, notify the sockIOPool to reload itself
			if (ccc != null)
				ccc(config);
		...
		// we sockIOPool and refcount the listeners here so we can safely dispose them when all clients are destroyed
