...
{
	/// <summary>
	/// Represents a Memcached node in the sockIOPool.
	/// </summary>
	...
	public class MemcachedNode : IMemcachedNode
	{
		...
		/// 
		/// If the server is back online, we'll ercreate the internal socket sockIOPool and mark the server as alive so operations can target it.
		/// </summary>
		...
		public bool Ping()
		{
			...
			try
			{
				// we could connect to the server, let's recreate the socket sockIOPool
				lock (SyncRoot)
				{
					...
					// it's easier to create a new sockIOPool than reinitializing a dead one
