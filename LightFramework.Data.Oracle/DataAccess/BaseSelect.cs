...
{
    /// <summary>
    /// BaseSelect类提供对Oracle数据库一些基本的查询操作的抽象基类。
    /// </summary>
    ...
    public abstract class BaseSelect<T> : IBaseSelect<T>
    {
        ...
        protected abstract T DataReaderToEntity(OracleDataReader dr,MetaDataTable metaDataTable,params string[] columnNames);
        ...
        public virtual bool IsExistWithCondition(string condition, params object[] parameterValues)
        {
            ...
            object result = OracleHelper.ExecuteScalar(this._connectionString, CommandType.Text, sqlCmd, parameters);
        ...
        /// <param name="columnNames">该实体对象中对应的数据库表的列名</param>
        /// <returns>实体对象集合</returns>
        protected virtual List<T> GetEntities(string sqlCmd, OracleParameter[] parameters, CommandType commandType, params string[] columnNames)
        {
        ...
        /// <param name="columnNames">该实体对象中对应的数据库表的列名</param>
        /// <returns>实体对象集合</returns>
        protected virtual List<TEntity> GetEntities<TEntity>(string sqlCmd, OracleParameter[] parameters, CommandType commandType,
            Func<OracleDataReader,MetaDataTable,string[], TEntity> drToEntityAction, params string[] columnNames)
        {
            List<TEntity> entities = new List<TEntity>();
            var metaDataTable = new MetaDataTable(typeof(TEntity), this._tableName);

            using (OracleDataReader dr = OracleHelper.ExecuteReader(this._connectionString, commandType, sqlCmd, parameters))
            {
                try
                {
                    while (dr.Read())
                ...
                catch (Exception ex)
                {
                    ...
                    System.Diagnostics.EventLog.WriteEntry("LightFramework.Data.Oracle", message);
                }
            }
        ...
        protected virtual OracleParameter[] FillParameters(object[] parameterValues)
        {
            ...
            OracleParameter[] parameters = new OracleParameter[parameterValues.Length];
        ...
        protected virtual void ExecuteByTransaction(List<SqlExpression> sqlExpressions, IsolationLevel isolationLevel)
        {
            ...
            using (OracleConnection oracleConnection = new OracleConnection(this._connectionString))
            {
                oracleConnection.Open();
                using (OracleTransaction oracleTransaction = oracleConnection.BeginTransaction(isolationLevel))
                {
                    try
                    {
                        sqlExpressions.ForEach(expr =>
                            OracleHelper.ExecuteNonQuery(oracleTransaction, CommandType.Text, expr.CommandText, expr.Parameters));
                        oracleTransaction.Commit();
                    }
                    catch (OracleException ex)
                    {
                        System.Diagnostics.EventLog.WriteEntry("LightFramework.Data.Oracle", ex.ToString());
                    }
                }
            }
        }
